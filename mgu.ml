...
module Mgu : MGU = struct
  ...
  let rec occurs f1 f2 = match f2 with
    ...
    | Variable(_) -> false
    | Operator(_, terms) -> any (occurs f1) terms;; 
  ...
  let rec find_disagrement f1 f2 = match f1, f2 with
    ...
    | Operator(name1, _), Operator(name2, _) when not (name1 = name2) -> NonUnifiable
    | Operator(_,  []), Operator(_, []) -> NoDisagrement
    | Operator(name1, t1::q1), Operator(name2, t2::q2) -> let d = find_disagrement t1 t2 
                                                                  in
                                                                  let op1 = operator name1 (List.length q1) q1
                                                                  and op2 = operator name2 (List.length q2) q2 in
                                                                  if d = NoDisagrement
                                                                  then
  ...
  let rec substitute dis f = match dis, f with
    ...
    | Dis(t1, mv), Operator(name, terms) -> operator name (List.length terms) (map (substitute dis) terms)
    | _, _ -> f;;
  ...
  let unifiable f1 f2 = match f1, f2 with
    | Predicate(name1, terms1), Predicate(name2, terms2)
         when (name1 = name2) -> let op1 = operator "_p" (List.length terms1) terms1
                                 and op2 = operator "_p" (List.length terms2) terms2
