...
module Kernel : KERNEL = struct
  ...
  let forallL seq = match seq with
    | SelectedL(left, right, Forall(Variable(x), form)) -> let p = principal seq in
                                                           let s = add (meta_replace p) left in
                                                           NonSelected(s, right) 
    | _ -> Invalid;;

  let forallR seq = match seq with
    | SelectedR(left, right, Forall(Variable(x), form)) -> let p = principal seq in
  ...
  let existsL seq = match seq with
    | SelectedL(left, right, Exists(Variable(x), form)) -> let p = principal seq in
                                                           let f, c = const_replace p in
  ...
  let existsR seq = match seq with
    | SelectedR(left, right, Exists(Variable(x), form)) -> let p = principal seq in
  ...
  let rec format_term term = match term with
    ...
    | Variable(name) -> name
    | Constant(name) -> name
    | Operator(name, _, terms) -> name ^ "(" ^ concatenate (map format_term terms)  ^ ")" ;;

  let rec format_formula formula = match formula with
    | Predicate(name, n, terms) when n = 0 -> name
    | Predicate(name, _, terms) -> name ^ "(" ^ concatenate (map format_term terms) ^ ")"
    ...
    | Forall(Variable(name), f1) -> "\\forall " ^ name ^ "." ^ format_formula f1
    | Exists(Variable(name), f1) -> "\\exists " ^ name ^ "." ^ format_formula f1
    | _ -> "NotImplemented";;
